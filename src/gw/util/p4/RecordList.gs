...
class RecordList implements List<Record>
{
  ...
  function foundBaseRevision(logEntry : P4Client.FileLogEntry) : boolean {
    return logEntry.Op == "add"
  }

  function setAllPendingRecords(logEntry : P4Client.FileLogEntry) {
    for (rec in this) {
      if (rec != null and !rec.hasFoundSourceRev()) {
